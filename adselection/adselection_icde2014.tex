...
% affiliations
\author{\IEEEauthorblockN{Zhen Wang}
\IEEEauthorblockA{School of Information Science and Technology\\
Sun Yat-sen University\\
Guangzhou, China\\
Email: wangzh56@mail2.sysu.edu.cn}
\and
\IEEEauthorblockN{Zhongyuan Wang}
\IEEEauthorblockA{Microsoft Research Asia\\
Beijing, China\\
Email: zhy.wang@microsoft.com}
\and
\IEEEauthorblockN{Haixun Wang}
\IEEEauthorblockA{Microsoft Research Asia\\
Beijing, China\\
Email: haixun.wang@microsoft.com}
\and
\IEEEauthorblockN{Jianlin Feng}
...
Sponsored search is the main source of revenue of a search engine.
  Advertisers bid on keywords for their advertisements, and search
  engines match user queries to relevant ads based on the bid keywords.
  Since both queries and bid keywords are short texts and cannot be
  modeled by the standard bag-of-words approach, most existing
  approaches exploit user behavior data (e.g., click-through data,
  session data, etc.) to fill the semantic gap in matching bid
  keywords 
  ...
  queries and bid keywords.  Leveraging a probabilistic taxonomy and a
  ...
  and billions of bid keywords, we create a semantic index of
  concepts: relevant bid keywords are selected for a given query by
  ...
  0.7 billion keywords.  Experiment results show the effectiveness of
  our approach.
\end{abstract}
...
\input{introduction_icde2014}
\input{relatedwork}
\input{external_resources}
\input{framework}
\input{conceptualization}
\input{candidates_generation}
%\input{semantic_match}
%\input{pruning}
\input{experiment}
\input{conclusion}
